# support-reply-public — Manual / 매뉴얼
...
bin/support-reply init --inbox myinbox

# (2) 아무 것도 없는 상태에서 dry-run으로 동작 확인
bin/support-reply run --inbox myinbox --dry-run --auto-approve

# (3) 결과 확인
bin/support-reply view --inbox myinbox     # 누적된 reference 보기
bin/support-reply metrics --inbox myinbox  # 라운드별 KPI 표
```
...
bin/support-reply config set send.from_email "support@yourcompany.com" --inbox myinbox
bin/support-reply config set inbox.label "고객문의" --inbox myinbox

# 라이브 라운드 실행 (대화형 승인)
bin/support-reply run --inbox myinbox
...
```bash
# 4개 소스 모두 dry-run 지원
bin/support-reply bootstrap --inbox myinbox web --urls https://yourco.com/faq
bin/support-reply bootstrap --inbox myinbox notion --token $NOTION_TOKEN
bin/support-reply bootstrap --inbox myinbox gmail --label 과거고객문의 --since 90d
bin/support-reply bootstrap --inbox myinbox channeltalk --export ./channeltalk_export.json
```
...
bin/support-reply config set classify.backend llm_anthropic --inbox myinbox
```
...
```bash
bin/support-reply config set learn.auto_case true --inbox myinbox
```
...
```bash
bin/support-reply init --inbox companyA
bin/support-reply init --inbox companyB
bin/support-reply run --inbox companyA --dry-run --auto-approve
bin/support-reply run --inbox companyB --dry-run --auto-approve
```
...
├── bin/support-reply              # CLI entrypoint
...
### 9. 역할별 사용법 (CS / BD / recruiting / partnerships / sales)
...
# 3. Inspect
bin/support-reply view --inbox myinbox      # accumulated reference
bin/support-reply metrics --inbox myinbox   # KPI table
```

### 4. Going live (requires gws CLI auth)

```bash
bin/support-reply config set send.from_email "support@yourco.com" --inbox myinbox
bin/support-reply config set inbox.label "Customer Inquiries" --inbox myinbox
...
bin/support-reply bootstrap --inbox myinbox web --urls https://yourco.com/faq
bin/support-reply bootstrap --inbox myinbox notion --token $NOTION_TOKEN
bin/support-reply bootstrap --inbox myinbox gmail --label past-tickets --since 90d
bin/support-reply bootstrap --inbox myinbox channeltalk --export ./export.json
```
