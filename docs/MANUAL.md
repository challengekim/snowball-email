...
핵심 메커니즘은 **inbox별 누적되는 markdown reference**입니다. 매 라운드 분류 패턴/사례/원칙이 `reference/`에 append되고, **분류기(classifier)가 그 누적된 패턴을 읽어 round-over-round로 정확도가 올라갑니다** (UNKNOWN 비율 감소). v0.1.x에서 이 부분만 진짜로 작동합니다 — **draft body 생성에는 reference가 아직 주입되지 않아 round 1과 round 100 본문이 동일**한 한국어 템플릿입니다. drafting 단계의 compound는 v0.2 마일스톤이며 [`CHANGELOG.md`](../CHANGELOG.md), [`README.md`](../README.md) "Status & Roadmap", [`SKILL.md`](../SKILL.md) §1.5에 동일하게 명시되어 있습니다.
...
The core mechanism is a **per-inbox accumulating markdown reference**. Every round appends classification patterns, cases, and principles to `reference/`, and **the classifier reads them, so classification accuracy improves round-over-round** (UNKNOWN rate drops). In v0.1.x that's the only part that actually compounds — **the drafter does not consult `reference/` yet, so round 1 and round 100 produce the same Korean-template body**. Drafting compounding is the v0.2 milestone; [`CHANGELOG.md`](../CHANGELOG.md), [`README.md`](../README.md) "Status & Roadmap", and [`SKILL.md`](../SKILL.md) §1.5 all carry the same disclosure.
