# support-reply-public — 설계 문서 (Compound Brain Edition)

> **목표**: studypie 전용으로 만들어진 `/support-reply`를 누구나 쓸 수 있는 공개 스킬로 일반화한다. 첫 사용은 평범하지만, 100회차에는 그 회사의 CS 베테랑처럼 답하는 **복리(compound) 에이전트**.
...
2. **Reference의 단일 진실 소스는 markdown 트리이며, 사용자는 항상 `_compiled.md` 평면 뷰로 전체를 한 화면에서 읽을 수 있다.** 내부적으로 `core.md` + `patterns/*.md` + `cases.jsonl`로 split되더라도 `support-reply compile`이 항상 read-only 평면 뷰를 재생성한다. 벡터 DB·외부 SaaS 의존성을 day 1에 도입하지 않는다 (마이그레이션 트리거는 §5에서 정의).
...
4. **Bootstrap은 옵셔널.** 새 사용자가 4개 소스 ingestion 없이도 바로 `/support-reply`를 호출할 수 있어야 한다. ingestion이 없으면 단지 첫 N회가 더 평범할 뿐이다.
...
| 옵션 | 위치 | 장점 | 단점 | 결정 |
|---|---|---|---|---|
| A1 | `~/.claude/skills/support-reply-public/inboxes/{inbox}/reference.md` | 스킬과 함께 이동, 사용자별 격리 | 백업/sync는 사용자 책임 | **채택** |
| A2 | `~/Documents/<vault>/40_Skills/support-reply/{inbox}/reference.md` | Obsidian sync 자동, 검색 가능 | vault 없는 사용자 배제, 경로 의존 | 기각 (vault 없는 90% 사용자 차단) |
| A3 | git repo (`~/.support-reply-data/{inbox}/reference.md`) | 버전 관리·diff·rollback 무료 | 첫 사용에 git init·remote 설정 강요 | 옵션으로 제공 (`--git`) |
...
| C2 | 사용자가 명시적으로 `/support-reply learn` 호출 | 비용 0 | 잊혀짐 → 복리 깨짐 | 보조 (수동 보강용) |
...
- **Mitigation**:
  ...
  - (b) **50라운드마다 재calibration 자동 권유** (`support-reply recalibrate`).
  - (c) `support-reply classifier-stats`로 tier 분포·confidence 분포·overrride율 시계열 추적. tier-A 비율이 첫 50라운드 동안 30% 미만으로 떨어지면 자동 경고.
...
- **Integration (실제 Gmail inbox 1개)**:
  - 테스트 inbox `support-reply-test@<도메인>`에 5종 카테고리 메일 시드.
...
- **Observability**:
  ...
  - `cost.json`은 누적집계 → `support-reply stats` 커맨드로 호출당 평균 토큰·달러·발송수 노출.
...
기존의 Smart Reply는 일반 톤 3개를 추천하고 끝난다. Help Scout/Front/Intercom은 SaaS 가입·티켓 마이그레이션·월 $50/seat을 요구한다. `support-reply-public`은:
...
### 0회차 (셋업 직후)
```bash
/oh-my-claudecode:plugin install support-reply-public
support-reply init --inbox brand_support \
  --label "Customer Support" \
  --from "support@brand.co" \
  --send drafts-only
```
- inbox 디렉토리·빈 reference·승인 게이트 설정 완료.
- bootstrap은 권유만 받고 skip 가능. 그래도 즉시 `/support-reply brand_support` 호출 가능.
...
### 100회차 (3개월 사용)
- reference 1700줄, 130KB → §5의 split 발동 → `core.md` + `patterns/*.md` + `cases.jsonl`로 분리됨. 사용자는 여전히 `support-reply view`(=`_compiled.md`) 한 화면에서 전체를 본다.
- 신규 패턴 발견율은 **측정값**(`support-reply metrics --growth`)으로만 판단. 3주 이동평균이 0.5/round 미만이면 시스템이 "수렴" 배지 부여, 그 전까지는 가속 가정.
- 이제 사용자는 승인 단계에서 "1, 3, 5번 발송, 2번 5만원→8만원으로, 4번 보류" 식의 라이트한 명령만 내림 (단, 이는 §8의 tier-B 저위험 라운드 기준).

### 측정 가능 KPI (마케팅 카피 금지)

매 라운드 종료 시 `inboxes/{inbox}/metrics.jsonl`에 한 줄 append. `support-reply metrics --window {10|50|100}`로 trailing 추세 출력.
...
1. `$SUPPORT_REPLY_DATA_DIR/{inbox}/` — 환경변수 우선. 머신 마이그레이션·oh-my-claudecode 업그레이드 시 사용자 데이터 보존을 위한 day-1 결정사항.
2. `--git` 플래그 → `~/.support-reply-data/{inbox}/` (자동 git init + 자동 commit `reference: append pattern X` + 사용자 remote push 옵션). 스킬은 symlink로 참조.
3. 디폴트 → `~/.claude/skills/support-reply-public/inboxes/{inbox}/`.

스킬 디렉토리에 사용자 데이터를 박아두면 스킬 재설치 시 데이터 분실 위험. 따라서 `init`이 데이터 디렉토리를 항상 stdout에 명시하고 `config.yaml`에 절대경로로 기록한다.

### 다중 inbox

`config.yaml` 단위로 격리. 한 사용자가 `brand_support`(한국어, gws_drafts), `client_b2b`(영어, sendgrid), `personal_freelance`(한국어, gws_send)를 동시에 운영해도 reference는 절대 섞이지 않음. 호출은 `support-reply run --inbox brand_support`.
...
```
support-reply bootstrap --inbox brand_support \
  --gmail-months 6 \
...
- **검토 게이트**: `bootstrap_pending.md`에 `### [Gmail #1] 환불 문의 — 전형 패턴` 식으로 적재. 사용자가 `/support-reply review-bootstrap brand_support` 호출 시 라인별 승인 (a/r/e=accept/reject/edit).
...
- **입력**: 사용자가 yaml 직접 편집할 일 없음. 두 경로:
  - 인터랙티브: `support-reply add-source notion --interactive` → 실행 시 `notion-search`로 후보 페이지 목록을 띄우고 사용자가 ✓로 1개씩 선택 → allowlist 자동 작성.
  - 비대화형(스크립트 친화): `support-reply config set bootstrap.notion.pages <id1> <id2>` 커맨드로 추가. 둘 다 결과적으로 `bootstrap.config.yaml`에 기록되지만 yaml은 산출물이지 입력이 아님.
...
```bash
support-reply add-source --inbox brand_support \
...
- `support-reply compile` 자동 호출 → `_compiled.md` (read-only, 사람이 grep·읽기 전용) **매 reference write 진입점에서** 자동 재빌드. file watcher 없이도 모든 write는 `reference_io` 단일 진입을 거치므로 race condition 없음.
- `_compiled.md` front-matter에 `source_hashes: {core.md: sha256, "patterns/01_refund.md": sha256, ...}` 기록. `view`/`grep` 호출 시 split 파일 mtime/sha256과 비교, 불일치 발견 시 "stale: rebuild needed (`support-reply compile --force`)" 에러로 차단해 사용자가 stale 사본으로 의사결정하지 않도록 함.
- 사용자 워크플로우 ("한 화면에서 전체 reference 보고 싶다") = `support-reply view` (= `_compiled.md` 페이저로 출력).
...
- 사용자가 `support-reply prune --dry-run`으로 후보 확인 후 실행.
...
1. **임계 50% 도달 (예: 100패턴 / 500KB / 25k 토큰)** → `support-reply migrate-dryrun` 자동 실행. 백그라운드에서 chroma 인덱스 빌드, 동일 시드 메일 50건에 대해 (markdown 분류 vs vector retrieval 분류) 정확도·토큰·지연 비교 리포트 생성. 사용자에게는 알림만, 액션은 없음.
...
- `support-reply teach --inbox brand_support` → 자유 markdown 입력 → reviewer LLM이 어느 섹션에 어떻게 들어가야 할지 제안 → 사용자 승인.
- `support-reply correct --inbox brand_support --pattern 5 "정산일을 매월 25일에서 말일로 변경"` → 패턴 5 본문 직접 패치 + changelog 자동 기록.

### 6.3 추적 / 롤백

- 모든 reference 변경은 `runs/{ts}/reference.diff` + (옵션) git commit.
- `support-reply rollback --runs 2026-04-25T10` → 그 시점 이후 모든 reference 변경 revert.
...
- 발송 후 사용자 클레임 0건 (`support-reply complaint` 명령으로 기록)
...
- **Calibration mode (첫 30라운드 강제)**: 신규 inbox 또는 reference 대량 변경 후 첫 30라운드는 분류기 출력 무시 + **모든 draft를 tier-A로 강제**. 사용자 manual override 비율(=정답률) 측정. **정답률 ≥ 95% 도달 시에만 calibration 종료**, tier-B 활성화. 95% 미달 시 calibration 30라운드 연장 + Critic Review 권유 (`support-reply classifier-stats`).
...
- `metrics.jsonl` **기록만** (읽기/표 출력은 W2). `$SUPPORT_REPLY_DATA_DIR` 인식.
...
- `support-reply stats` (라운드 수·발송 수·평균 토큰·달러) + `metrics --window {10|50|100}` + `metrics --growth` (3주 이동평균 KPI).
...
- **deliverable 커맨드**: `support-reply graduation-stats` 신설 (위 통계 + 권장 임계 출력).
...
- `bootstrap_pending.md` + `support-reply review-bootstrap` 커맨드.
- Reviewer LLM PII 스캔.
- §7.3 `gws_drafts → gws_send` 자동 졸업 게이트 활성화 (W2에서 보정된 임계 적용).
- `support-reply classifier-stats` + `support-reply recalibrate` (50라운드 자동 권유).

### W4 — Bootstrap (Notion + ChannelTalk) + Pruning
- `ingest_notion.py`, `ingest_channeltalk.py`.
- `support-reply prune` + archive.
...
| 항목 | Gmail Smart Reply | Help Scout / Front | Intercom Copilot | **support-reply-public** |
|---|---|---|---|---|
...
- **배포 채널**:
  1. `oh-my-claudecode plugin` 마켓 등록 (`/oh-my-claudecode:plugin install support-reply-public`).
  2. GitHub `taewookim/support-reply-public` (or org). README는 영문 + 한국어.
  3. 설치 후 `support-reply doctor` 명령으로 의존성 자동 점검 (gws cli 설치 여부, Python ≥ 3.10).
...
- **R4** (의도적 미해결, fail loud): gws cli 자체 의존. gws cli deprecate / OAuth 만료 시 `support-reply run`은 명확한 에러로 종료한다 ("gws cli 인증 만료. `gws auth login` 후 재시도"). silent fallback·자동 SDK 전환은 **하지 않는다** — Principle 3의 비협상성 보존. 사용자가 명시적으로 `--allow-sdk-fallback` 옵션을 옵트인하지 않는 한 다른 Gmail 라이브러리로 전환하지 않는다. 옵트인은 1회성, 매 실행마다 경고 표시.
...
1. **(W1, day 1)** `scripts/support_reply.py`에 `init` + `run` + `view` + `grep` + `diff` 5개 커맨드 골격 작성. `$SUPPORT_REPLY_DATA_DIR` 우선 인식. `inboxes/<name>/config.yaml`(산출물) + 빈 `reference.md` 생성. 기존 `assets/send_replies.py`의 SendGrid 코드는 `senders/sendgrid_send.py`로 이식 (SendBackend ABC 적용). `gws_drafts` backend 우선 구현 (MVP는 drafts-only, `gws_send` 졸업은 W3).
...
3. **(W2, day 1–2)** `learner.py` 자동 diff 제안 + 다단계 Jaccard dedup (0.85 자동 / 0.5–0.85 confirm / <0.5 별도) + 사용자 승인 + `reference_io.apply_diff()` 라운드 종료 후 자동 호출. MVP 종결 전 `support-reply stats` + `metrics --growth` 추가.
...
- **Pin-3 (W2 dogfooding deliverable)**: §10 W2에 dogfooding 산출물 명시 — ≥30라운드 metrics 수집 → 75p 계산 → graduation 임계치 자동 재설정, `support-reply graduation-stats` 커맨드 + `dogfood_log.md` 결과물 deliverable화. W3은 graduation 활성화 단계로 분리.
...
- **VIOL-1 (Principle 2 자가 위반)**: §RALPLAN-DR Principle 2 재서술 — "단일 진실 소스는 markdown 트리이며 사용자는 항상 `_compiled.md` 평면 뷰로 전체를 한 화면에서 본다". §5.2에 `support-reply compile` 자동 재생성 + read-only `_compiled.md` 명시. §10 W1에 `view`/`grep`/`diff` 커맨드 day-1 포함.
- **VIOL-2 (Principle 5 위반)**: §4.3 노션 yaml 직접 편집 금지 → `support-reply add-source notion --interactive`(검색→체크박스) 또는 `support-reply config set bootstrap.notion.pages <id>` 커맨드. yaml은 산출물.
...
- **REQ-1 (수렴 측정 KPI)**: §2에 `### 측정 가능 KPI` 표 추가 — `edit_ratio`/`unknown_rate`/`new_pattern_rate`/`tier_b_handtime` 4개. 100회차 narrative에서 "수렴" 단정 삭제, 측정값으로만 판정. `metrics.jsonl` + `support-reply metrics --window` 명세.
...
- **REQ-5 (벡터 DB 점진 신뢰)**: §5.5를 2단계로 — 임계 50% 도달 시 `support-reply migrate-dryrun` 자동 백그라운드 실행 + 비교 리포트, 100% 도달 시 "이미 검증된 마이그레이션" 권유.
- **REQ-6 (W1 view/grep/diff)**: §10 W1 deliverable에 `view`/`grep`/`diff` 3개 커맨드 day-1 필수 포함. §14 첫 액션 1번에도 반영.
- **REQ-7 (데이터 디렉토리 분리)**: §3 "데이터 위치 결정" 재작성 — `$SUPPORT_REPLY_DATA_DIR` 우선, 그다음 `--git`, 마지막이 디폴트. init 시 절대경로 stdout + config.yaml 기록.
...
- **VIOL-1 (Principle 2 자가 위반)**: §5.2 split이 "한 개 markdown이 진실"을 자동으로 깨뜨림. 심각도 中 — split 후에도 사람 가독성 유지하려면 최소한 `view` 커맨드(`support-reply view --pattern 5`)가 day 1부터 있어야 하는데 §10 로드맵에 없음.
...
- **[REQ-1] §2 100회차 narrative** — current: "신규 패턴은 한 라운드당 0–1개 수준으로 안정화 (수렴)" → propose: "라운드당 신규 패턴 N개 측정값을 KPI로 노출(`support-reply stats --growth`), 3주 이동평균이 0.5/round 미만이면 '수렴' 판정. 그 전까진 수렴 주장 금지" — reason: production reference는 수렴은커녕 가속 중. 측정 없이 단정하면 §11 차별점(compound)이 검증 불가능한 마케팅 카피로 전락.
...
- **[REQ-4] §4.3 노션 allowlist** — current: `bootstrap.config.yaml`에 page_ids 사전 명시 → propose: `support-reply add-source notion --interactive`로 노션 검색 → 사용자가 페이지 1개씩 + 버튼 클릭 → allowlist 자동 작성. yaml 직접 편집 금지 — reason: VIOL-2 해소.
...
- **[REQ-6] §10 W1** — current: W1에 `view` 커맨드 없음 → propose: W1에 `support-reply view --pattern <id|slug>` + `support-reply grep <regex>` 필수 포함 (split 이후 사람 가독성 유지) — reason: VIOL-1 해소, Principle 2 보존.

- **[REQ-7] §3 디렉토리** — current: `~/.claude/skills/support-reply-public/inboxes/{inbox}/` → propose: 디폴트는 동일하되 init 시 환경변수 `SUPPORT_REPLY_DATA_DIR` 우선 인식. 머신 마이그레이션 시 "스킬은 재설치, 데이터는 원래 자리"가 가능해야 함 — reason: 스킬 디렉토리에 사용자 데이터가 박히면 oh-my-claudecode 업그레이드 시 데이터 분실 위험. 분리가 day 1 결정사항.
...
- **S3 (VIOL-1 해소)**: split 후에도 단일 진실 보존을 위해 `support-reply compile` 커맨드로 `core.md` + `patterns/` → `_compiled.md` 평면 뷰 자동 생성 (read-only, 매 변경 시 재생성). 사람은 `_compiled.md`를 grep, 시스템은 split 파일을 lazy load. Principle 2 정신 살림.
...
- **REQ-7 DATA_DIR init-time 고정**: 머신 마이그 시 사용자가 `support-reply migrate-data --to <new_path>` 같은 명시 커맨드 필요. 현 PLAN에 없음. 미래 R로 분류 가능.
...
2. **W2 dogfooding deliverable (Pin-3)** — ✅ 해소
   ...
   - `support-reply graduation-stats` CLI 신설로 사용자가 자기 데이터로 임계 재조정 가능
...
**support-reply-public PLAN.md iter3 — APPROVE for ralph W1 execution. 모든 Principle violation·tradeoff tension·incentive misalignment가 measurable mechanism으로 봉인됨.**

### Executive Summary

**무엇을 만드는가**: studypie 전용 `/support-reply`를 누구나 쓸 수 있게 일반화한 compound CS 스킬. 단일 markdown reference + gws cli fetch + plug-in sender(W1은 `gws_drafts`만) + 비활성화 불가 2-tier 승인 게이트(calibration mode 30R + confidence < 0.85 시 tier-A 강등 + (c)(d) regex AND LLM dual-gate) + 라운드 종료 자동 LLM diff 학습.
...
- **2026-04-26 phase B**: W1 MVP 산출물 작성 (SKILL.md, bin/support-reply, config/schema.yaml, ingest/{_common,gmail,web,notion,channeltalk}.py, approval/tier_classifier.py, metrics/logger.py, README.md) — verifier sign-off pending
