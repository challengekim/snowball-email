...
├── bin/snowball-email                # CLI dispatcher (init/config/bootstrap/view/grep/diff/metrics/run)
...
**SNOWBALL_EMAIL_HOME** 환경변수로 위치 override 가능. 디폴트는 `~/.claude/skills/snowball-email/`.
...
3. **Metrics 기반 자가 진단** — `snowball-email metrics`로 edit_ratio·unknown_rate·new_pattern_rate 추세 확인. PLAN §2 KPI threshold 위반 시 경고.
4. **View/grep/diff CLI** — `snowball-email view`로 누적된 reference 평탄 출력, `grep`으로 특정 패턴 검색, `diff`로 round 간 변경 가시화.
...
```bash
# 최초 1회
snowball-email init                          # 인터랙티브 wizard (yaml 노출 X)
snowball-email bootstrap gmail --inbox support
snowball-email bootstrap web --inbox support
snowball-email bootstrap notion --inbox support
snowball-email bootstrap channeltalk --inbox support

# 매일 사용
snowball-email run --inbox support           # 1 round (fetch → classify → draft → approve → send → log)
snowball-email run --inbox support --dry-run # 발송 없이 초안만 보기

# Compound 가시화
snowball-email view --inbox support          # _compiled.md 출력
snowball-email grep '환불' --inbox support   # reference 검색
snowball-email diff --inbox support          # 직전 round 대비 reference 변경
snowball-email metrics --inbox support --last 50  # 최근 50 round KPI 추이

# 설정
snowball-email config get send.backend
snowball-email config set send.backend gws_send
```
